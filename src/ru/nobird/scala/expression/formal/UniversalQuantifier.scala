...
/**
  ...
  */
class UniversalQuantifier(v: String, body: Expression) extends Expression("@" + v + "(" + body.toString + ")") {
    override def execute(vars: Map[String, Boolean]): Option[Boolean] = None
    override def getType: ExpressionType = ExpressionType.UniversalQuantifier

    override def getChild: List[Expression] = List(body)
    override def insertExpression(vars: Map[String, Expression]): Expression = new UniversalQuantifier(v, body.insertExpression(vars))
    ...
    override def getLinkedVariablesOnWayToX(x: String, current: Set[String]): Set[String] = (Set[String]() /: getChild)((s, c) => s ++ c.getLinkedVariablesOnWayToX(x, current + v))
